...
public class BillingServiceStoreEmployeeIntegrationTest extends TestData {
	...
	public void setUp() {
		...
		User user = new User("KetulStoreEmployee", "STORE_EMPLOYEE", new Date());
		user.setId(userId);
	...
	public void test_getDiscountByUserAndEligibleAmount_ForAffilliateUser() throws Exception {
		...
		Double eligibleAmount = billingService.getTotalEligibleAmountForDiscount(products);
		Double eligibleDiscount = (eligibleAmount * BillingConstants.storeEmployeeDiscountPercent) / 100;
		// when
